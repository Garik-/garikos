...
const leftEye = useTemplateRef('leftEye')
const rightEye = useTemplateRef('rightEye')
const wrapper = useTemplateRef('wrapper')

function onMousemove(e: MouseEvent) {
  if (!leftEye.value || !rightEye.value || !e) {
    return
  }

  const leftRect = leftEye.value.getBoundingClientRect()
  ...
  leftEye.value.style.transform = 'rotate(' + rot + 'deg)'
  rightEye.value.style.transform = 'rotate(' + rot + 'deg)'
}

onMounted(() => {
  window.addEventListener('mousemove', onMousemove)

  wrapper.value?.classList.remove('loading')
  wrapper.value?.classList.add('loaded')
})
...
<template>
  <div class="gopher-wrapper loading" ref="wrapper">
...
.gopher {
  ...
  .eye-socket {
    ...
    &.right {
      ...
      width: 16px;
      height: 21px;
      border-radius: 14px 100% 15px 100%;
      .eye {
        right: 0px;
      }
    }
